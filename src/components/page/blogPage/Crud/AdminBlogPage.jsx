...
const AdminBlogPage = () => {
  ...
  const [categories, setCategories] = useState([]);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [showStatusModal, setShowStatusModal] = useState(null); // تغییر نام از showPublishModal
  const [selectedPost, setSelectedPost] = useState(null);
  const [actionLoading, setActionLoading] = useState(false);
  const [pagination, setPagination] = useState({
    pageNumber: 1,
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`${API_BASE_URL}/Post/DeleteStatus`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify(selectedPost.id)
      });
      ...
      console.error('خطا در حذف مقاله:', error);
      setError('خطا در حذف مقاله: ' + error.message);
    } finally {
      setActionLoading(false);
    }
  };

  // **تغییر وضعیت مقاله با API جدید**
  const handleToggleStatus = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const newStatus = !selectedPost.isPublished;
      console.log(newStatus,selectedPost.id)
        // ارسال به صورت جداگانه - روش 1: استفاده از URLSearchParams
    const formData = new URLSearchParams();
    formData.append('id', selectedPost.id);
    formData.append('type', newStatus);
      const response = await fetch(`${API_BASE_URL}/Post/UpdateStatus`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (result.status === 200) {
        ...
        setSelectedPost(null);
        showSuccess(newStatus ? 'مقاله با موفقیت منتشر شد' : 'مقاله با موفقیت غیرفعال شد');
      } else {
        throw new Error(result.message || 'خطا در تغییر وضعیت مقاله');
      }
    } catch (error) {
      console.error('خطا در تغییر وضعیت:', error);
      setError('خطا در تغییر وضعیت مقاله: ' + error.message);
    } finally {
      setActionLoading(false);
    }
  };
