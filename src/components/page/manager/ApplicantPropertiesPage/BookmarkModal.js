...
import { FaBookmark, FaTimes, FaSave, FaEdit, FaTrash, FaStickyNote } from 'react-icons/fa';
import './BookmarkModal.css';

const BookmarkModal = ({ isOpen, onClose, property, onBookmarkSaved }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.status === 200) {
          ...
          alert('بوکمارک با موفقیت ذخیره شد');
          onClose();
        }
      }
    } catch (error) {
  ...
  const handleDeleteBookmark = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBookmarkId(null);
        if (onBookmarkSaved) {
          onBookmarkSaved(property.id, null);
        }
