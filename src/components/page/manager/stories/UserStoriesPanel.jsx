...
// //     const [storyViewIndex, setStoryViewIndex] = useState(0);
// //     const [storyProgress, setStoryProgress] = useState(0);
// //     const [tempCacheIds, setTempCacheIds] = useState([]); // برای ذخیره cacheId ها
...
// //             // آپلود خودکار عکس به سرور و دریافت cacheId
// //             try {
// //                 setUploading(true);
// //                 const result = await panelService.UploadTempImage(file);
// //                 if (result.success) {
// //                     setTempCacheIds(prev => [...prev, result.cacheId]);
// //                     showUsrToast('عکس با موفقیت آپلود شد', 'success');
// //                 }
// //             } catch (error) {
// //                 console.error('Error uploading image:', error);
// //                 showUsrToast('خطا در آپلود عکس', 'error');
// //             } finally {
// //                 setUploading(false);
// //             }
// //         }
...
// //     const removeImage = () => {
// //         // پاک کردن آخرین cacheId
// //         if (tempCacheIds.length > 0) {
// //             const lastCacheId = tempCacheIds[tempCacheIds.length - 1];
// //             panelService.ClearTempImage(lastCacheId).catch(console.error);
// //             setTempCacheIds(prev => prev.slice(0, -1));
// //         }
// //         setStoryImage(null);
// //         setStoryImagePreview(null);
// //     };
...
// //         // اگر عکس انتخاب شده ولی هنوز آپلود نشده
// //         if (storyImage && tempCacheIds.length === 0) {
// //             showUsrToast('لطفاً منتظر بمانید تا عکس آپلود شود', 'error');
// //             return;
// //         }

// //         if (!storyText && tempCacheIds.length === 0 && !selectedRealEstate) {
// //             showUsrToast('لطفاً حداقل متن، عکس یا ملک را وارد کنید', 'error');
// //             return;
// //         }
...
// //         try {
// //             // داده‌های استوری رو با لیست cacheId ها می‌فرستیم
// //             const storyData = {
// //                 title: storyText || null,
// //                 realEstateId: selectedRealEstate || null,
// //                 tempImageCacheIds: tempCacheIds // لیست cacheId ها
// //             };
...
// //             setStoryImagePreview(null);
// //             setSelectedRealEstate('');
// //             setTempCacheIds([]); // خالی کردن لیست cacheId ها
// //             setShowAddModal(false);
// //             fetchStories();
...
// //                                             <div className="usr-image-preview-overlay">
// //                                                 <span>📷 عکس شما</span>
// //                                                 {tempCacheIds.length > 0 && (
// //                                                     <span className="usr-upload-status">✓ آپلود شد</span>
// //                                                 )}
// //                                             </div>
// //                                         </div>
// //                                     ) : (
...
// //                                             <div className="usr-upload-content">
// //                                                 <span className="usr-upload-icon">📸</span>
// //                                                 <span>{uploading ? 'در حال آپلود...' : 'برای آپلود عکس کلیک کنید'}</span>
// //                                                 <small>jpg, png, webp - حداکثر 10MB</small>
// //                                             </div>
// //                                         </label>
// //                                     )}
// //                                 </div>
// //                                 {tempCacheIds.length > 0 && (
// //                                     <div className="usr-uploaded-files">
// //                                         <span>✅ {tempCacheIds.length} عکس آپلود شد</span>
// //                                     </div>
// //                                 )}
// //                             </div>
...
//     const [uploading, setUploading] = useState(false);
//     const [usrToast, setUsrToast] = useState(null);
//     const [properties, setProperties] = useState([]); // لیست املاک
//     const [selectedStoryForView, setSelectedStoryForView] = useState(null);
//     const [storyViewIndex, setStoryViewIndex] = useState(0);
//     const [storyProgress, setStoryProgress] = useState(0);
//     const [tempCacheIds, setTempCacheIds] = useState([]);
...
//     // دریافت لیست املاک برای کامبوباکس
//     const fetchProperties = useCallback(async () => {
//         try {
//             const data = await panelService.GetRealEstatePanelForStory();
//             console.log('🏠 Properties loaded:', data);
//             if (data && Array.isArray(data)) {
//                 // فیلتر کردن املاک فعال
...
//         } catch (error) {
//             console.error('Error fetching properties:', error);
//             showUsrToast('خطا در دریافت لیست املاک', 'error');
//         }
//     }, []);
...
//                 realEstateId: selectedRealEstate || null,
//                 urlAddress:"",
//                 tempImageCacheIds: tempCacheIds
//             };
...
//                                         value={selectedRealEstate}
//                                         onChange={(e) => setSelectedRealEstate(e.target.value)}
//                                         className="usr-property-select-input"
//                                     >
//                                         <option value="">بدون ملک</option>
//                                         {properties.length > 0 ? (
//                                             properties.map(property => (
//                                                 <option key={property.id} value={property.id}>
//                                                     🏠 {property.title} - {property.region || 'منطقه نامشخص'}
//                                                 </option>
//                                             ))
//                                         ) : (
//                                             <option value="" disabled>در حال بارگذاری املاک...</option>
//                                         )}
//                                     </select>
//                                 </div>
//                                 {properties.length === 0 && (
//                                     <small className="usr-property-loading">
//                                         ⏳ در حال دریافت لیست املاک...
//                                     </small>
//                                 )}
//                             </div>
...
const UserStoriesPanel = () => {
    ...
    // وقتی ملک انتخاب میشه، عکس‌ها رو پاک کن
    const handlePropertySelect = (e) => {
        const propertyId = e.target.value;
        setSelectedRealEstate(propertyId);
        
        // اگر ملک انتخاب شد، عکس‌ها رو پاک کن
        if (propertyId) {
            // پاک کردن همه عکس‌های موقت
            tempCacheIds.forEach(cacheId => {
                panelService.ClearTempImage(cacheId).catch(console.error);
            });
            setTempCacheIds([]);
            setStoryImage(null);
            setStoryImagePreview(null);
            showUsrToast('با انتخاب ملک، عکس‌ها حذف شدند', 'info');
        }
    };
    ...
    // بررسی اینکه آیا ملک انتخاب شده
    const isPropertySelected = !!selectedRealEstate;
                    ...
                    <div className="usr-stories-modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmitStory}>
                            ...
                            <div className="usr-stories-form-group">
                                ...
                                {isPropertySelected && (
                                    <div className="usr-property-selected-info">
                                        ✅ ملک انتخاب شده - امکان آپلود عکس وجود ندارد
                                    </div>
                                )}
                            </div>
                            
                            {/* بخش آپلود عکس - فقط در صورتی که ملک انتخاب نشده باشه نمایش داده میشه */}
                            {!isPropertySelected && (
                                <div className="usr-stories-form-group">
                                    <label>افزودن عکس</label>
                                    <div className="usr-image-upload">
                            ...
                            {/* اگر ملک انتخاب شده، پیام جایگزین نمایش داده میشه */}
                            {isPropertySelected && (
                                <div className="usr-stories-form-group">
                                    <div className="usr-property-mode-message">
                                        <div className="usr-property-mode-icon">🏠</div>
                                        <div className="usr-property-mode-text">
                                            <h4>حالت نمایش ملک</h4>
                                            <p>با انتخاب ملک، استوری شما به صورت نمایش ملک ثبت میشود</p>
                                            <div className="usr-property-mode-preview">
                                                <div className="usr-preview-item">
                                                    <span>📌</span>
                                                    <span>ملک: {properties.find(p => p.id === parseInt(selectedRealEstate))?.title || 'انتخاب شده'}</span>
                                                </div>
                                                <div className="usr-preview-item">
                                                    <span>📍</span>
                                                    <span>منطقه: {properties.find(p => p.id === parseInt(selectedRealEstate))?.region || 'نامشخص'}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
