...
const AddPropertyModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handlePriceChange = (e) => {
    const rawValue = e.target.value.replace(/[^\d]/g, '');
    setDisplayPrice(formatNumberWithCommas(rawValue));
    // حذف تبدیل به میلیون - ذخیره مستقیم همان عدد
    setFormData(prev => ({ ...prev, price: rawValue }));
  };
  ...
  const uploadSingleImage = async (file, previewUrl) => {
    ...
    try {
      ...
      console.error('خطا در آپلود تصویر:', error);
      setUploadStatus(prev => new Map(prev).set(previewUrl, 'error'));
      showToast(`خطا در آپلود تصویر: ${error.message}`, 'error');
      return null;
    }
  };
  ...
  const handleImageChange = async (e) => {
    ...
    for (const file of files) {
      if (file.size > 10 * 1024 * 1024) {
        showToast(`حجم فایل ${file.name} بیشتر از 10 مگابایت است`, 'error');
        continue;
      }
      
      if (!file.type.startsWith('image/')) {
        showToast(`فایل ${file.name} از نوع تصویر نیست`, 'error');
        continue;
      }
  ...
  const addAmenity = (amenity) => {
    ...
    if (!amenityObj || !amenityObj.id) {
      showToast('این ویژگی در لیست موجود نیست!', 'error');
      return;
    }
  ...
  const validateForm = () => {
    if (!formData.title.trim()) {
      showToast('لطفاً عنوان ملک را وارد کنید', 'error');
      return false;
    }
    ...
    if (!formData.area || parseInt(formData.area) === 0) {
      showToast('لطفاً متراژ ملک را وارد کنید', 'error');
      return false;
    }
    
    if (!locationSelected) {
      showToast('لطفاً موقعیت ملک را روی نقشه انتخاب کنید', 'error');
      return false;
    }
    
    if (!selectedTransaction) {
      showToast('لطفاً نوع معامله را انتخاب کنید', 'error');
      return false;
    }
    
    if (documentType === null) {
      showToast('لطفاً نوع سند را انتخاب کنید', 'error');
      return false;
    }
    
    const hasUploading = Array.from(uploadStatus.values()).some(status => status === 'uploading');
    if (hasUploading) {
      showToast('لطفاً منتظر بمانید تا تصاویر آپلود شوند...', 'error');
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // حذف ضرب در 10000 - ذخیره دقیقاً همان عدد وارد شده
      let finalPrice = 0;
      if (activeTab === 'buy') {
        finalPrice = parseInt(formData.price) || 0;
      } else {
        finalPrice = parseInt(rentalData.rentPrice) || 0;
      }
      ...
      const submitData = {
        ...
        categoryTypeId: formData.categoryTypeId,
        price: finalPrice,
        rentPrice: activeTab === 'rent' ? (parseInt(rentalData.rentPrice) || 0) : 0,
        depositPrice: activeTab === 'rent' ? (parseInt(rentalData.depositPrice) || 0) : 0,
        isMortgageOnly: activeTab === 'rent' ? rentalData.isMortgageOnly : false,
        contractDuration: constructionYear,
        tempImageCacheIds: cacheIdsOnly,
        isRenovated: isRenovated,
      ...
      console.error('Error adding property:', error);
      showToast(error.message || 'خطا در ثبت ملک', 'error');
    } finally {
      setIsSubmitting(false);
      setLoading(false);
    }
            ...
            <div className="add-form-row">
              {activeTab === 'buy' ? (
                <div className="add-form-group">
                  ...
                  <small>مبلغ را به تومان وارد کنید (مثال: 1,500,000)</small>
                </div>
              ) : (
                <>
                  <div className="add-form-group">
