...
const EditPropertyModal = ({ isOpen, onClose, onSuccess, propertyId }) => {
  ...
  // دریافت اطلاعات ملک برای ویرایش
  const fetchPropertyData = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://localhost:7178/api/RealEstatePage/GetRealEstateDetailsForEdit?id=${propertyId}`, {
        method: 'GET',
        headers: {
      ...
      if (result.status === 200 && result.data) {
        ...
        // تعیین تب فعال بر اساس categoryType
        const isRent = data.categoryType === 2;
        setActiveTab(isRent ? 'rent' : 'buy');
        
        // تنظیم نوع معامله
        setSelectedTransaction({
          id: data.categoryType === 1 ? 1 : 3,
          name: data.categoryType === 1 ? 'فروش نقدی' : 'رهن کامل',
          categoryTypeId: data.categoryType
        });
        
        // تنظیم فرم دیتا
        setFormData({
          title: data.title || '',
          price: data.price ? (data.price / 10000).toString() : '',
          area: data.priceMeter ? (data.price / data.priceMeter).toString() : '',
          address: data.address || '',
          rooms: data.rooms || 0,
          unitsPerFloor: 0,
          countFloor: data.countFloor || 1,
          floor: data.floor || 1,
          hasParking: data.isHasParking || false,
          hasElevator: data.isHasElevator || false,
          hasLoan: data.isHasLoan || false,
          description: data.descriptionRows || '',
          lat: data.lat || 35.699739,
          lng: data.lng || 51.338097,
          showExactLocation: data.showExactLocation ?? true,
          categoryTypeId: data.categoryType,
          transactionTypeId: data.categoryType === 1 ? 1 : 3,
        });
        
        // تنظیم قیمت نمایشی
        if (data.price) {
          setDisplayPrice(formatNumberWithCommas((data.price / 10000).toString()));
        }
        
        // تنظیم دیتای اجاره
        if (data.categoryType === 2) {
          setRentalData({
            rentPrice: data.rent ? (data.rent / 10000).toString() : '',
            depositPrice: data.deposit ? (data.deposit / 10000).toString() : '',
            isMortgageOnly: data.rent === 0,
            contractDuration: 12,
          });
        }
        
        // تنظیم سال ساخت
        if (data.constructionYear && data.constructionYear > 0) {
          setConstructionYear(data.constructionYear.toString());
        }
        ...
        if (data.facilities && data.facilities.length > 0) {
          const amenitiesWithId = data.facilities.map((facility, index) => ({
            id: index + 1,
            name: facility
          }));
          setSelectedAmenities(amenitiesWithId);
        }
        ...
        console.error('خطا در دریافت اطلاعات ملک:', result.message);
      }
    } catch (error) {
  ...
  const removeNewImage = async (index) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://localhost:7178/api/RealEstatePage/UpdateRealEstate', {
        method: 'PUT',
        headers: {
      ...
      if (result.status === 200) {
        ...
        throw new Error(result.message || 'خطا در ویرایش ملک');
      }
    } catch (error) {
      console.error('Error editing property:', error);
      alert(error.message || 'خطا در ویرایش ملک');
    } finally {
      ...
      <div className="add-modal-container" ref={modalRef}>
        ...
        <form onSubmit={handleSubmit} className="add-modal-form">
          ...
          <div className="add-form-group full-width">
            ...
            >
              {showCitySelector ? '🔽 بستن انتخاب شهر' : '📍 تغییر شهر'}
            </button>
          </div>
          
          {/* سیلکتور شهر - فقط در صورت نیاز نمایش داده می‌شود */}
          {showCitySelector && (
            <div className="add-form-group full-width">
          ...
          <div className="add-form-group full-width">
              ...
              <div className="existing-images-section">
                ...
                <div className="add-image-previews">
                  {existingImages.map((image, index) => (
                    <div key={`existing-${index}`} className="add-image-preview existing">
                      <img src={`https://localhost:7178${image}`} alt={`تصویر ${index + 1}`} />
                      <button 
                        type="button" 
            ...
            <div className="add-image-upload-area">
              <input 
                ...
                disabled={isSubmitting} 
              />
              <label htmlFor="edit-image-upload" className="add-upload-label">
                📸 افزودن تصاویر جدید
              </label>
              <div className="add-image-previews">
