...
const MainLayout = () => {
  ...
  // لوگوی پیش‌فرض (متن) در صورت عدم بارگذاری تصویر لوگو
  const defaultLogo = () => (
  ...
  return (
    <div className="main-layout">
      ...
      <header className={`main-header ${scrolled ? 'scrolled' : ''}`}>
        <div className="header-container">
          {/* لوگو - با قابلیت تغییر تصویر */}
          <div className="logo" onClick={() => navigate('/')}>
            {!logoError ? (
              <img 
                src="/image/logo.png"  // مسیر لوگوی شما - می‌توانید تغییر دهید
                alt="خونه‌یاب"
        ...
        <div className={`mobile-menu ${isMobileMenuOpen ? 'open' : ''}`}>
          ...
          <div className="mobile-auth">
            {user ? (
              <button className="mobile-login" onClick={() => navigate('/dashboard')}>
                <span>👤</span>
                <span>{user?.username || 'کاربر'} 👋 پنل کاربری</span>
              </button>
            ) : (
      ...
      <footer className="main-footer">
        <div className="footer-container">
          ...
          <div className="footer-section">
            <h4>تماس با ما</h4>
            <p>📞 ۰۲۱-۱۲۳۴۵۶۷۸</p>
            <p>✉️ info@khoneyab.ir</p>
          </div>
        </div>
        <div className="copyright">
          تمامی حقوق برای خونه‌یاب محفوظ است © ۱۴۰۴
        </div>
      </footer>
